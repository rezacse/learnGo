...
type Server interface {
	...
	UpdateCustomerById(ctx echo.Context) error
...
func (s *EchoServer) Start() error {
	if err := s.echo.Start(":8081"); err != nil && err != http.ErrServerClosed {
		log.Fatalf("server shoutdow occurred: %s", err)
		return err
	}
	return nil
}
...
func (s *EchoServer) registerRoutes() {
	...
	cg.PUT("/:id", s.UpdateCustomerById)
	cg.DELETE("/:id", s.DeleteCustomerById)
...
func (s *EchoServer) Readiness(ctx echo.Context) error {
	ready := s.DB.Ready()
	if ready {
		return ctx.JSON(http.StatusOK, entities.Health {Status: "OK"})
	}
	return ctx.JSON(http.StatusInternalServerError, entities.Health{Status: "Failure"})
}
